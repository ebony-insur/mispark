...
export default function Navbar() {
  ...
  useEffect(() => {
    const fetchAuth = async () => {
      ...
      if (user) {
        const { data } = await supabase.from("parent_profiles").select("sparks_balance, subscription_tier").eq("id", user.id).single();
        if (data) {
  ...
  return (
    <div className="w-full max-w-5xl flex justify-between items-center bg-white p-4 rounded-2xl shadow-sm border border-slate-200 print:hidden mb-8">
      <div className="flex items-center gap-4">
        ...
        <Image src="/MiSpark.svg" alt="MiSpark Logo" width={120} height={40} className="cursor-pointer" onClick={() => router.push("/")} />
      </div>
      ...
      <div className="flex gap-3 items-center">
        {isGuest ? (
          ...
          <>
            <button onClick={() => router.push("/billing")} className="flex items-center gap-1 bg-amber-50 text-amber-800 px-4 py-2 rounded-xl border border-amber-200 font-bold text-sm transition-colors hover:bg-amber-100">
            ...
            <div className="relative">
              ...
              {isAccountMenuOpen && (
                <div className="absolute right-0 mt-2 w-56 bg-white border border-slate-200 rounded-xl shadow-xl py-2 z-50">
                  <button onClick={() => router.push("/dashboard")} className="w-full text-left px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50">Dashboard</button>
                  <button onClick={() => router.push("/dashboard/students")} className="w-full text-left px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50">My Students</button>
                  <button onClick={() => router.push("/history")} className="w-full text-left px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50">My History</button>
                  <button onClick={() => router.push("/billing")} className="w-full text-left px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50">Manage Account</button>
                  <div className="h-px bg-slate-100 my-1"></div>
                  <button onClick={() => { supabase.auth.signOut(); router.push("/"); }} className="w-full text-left px-4 py-3 text-sm font-bold text-red-600 hover:bg-red-50">Sign Out</button>
                </div>
              )}
            </div>
          </>
