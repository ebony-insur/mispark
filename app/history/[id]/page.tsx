...
export default function HistoryDetailPage() {
  ...
  // Array to hold references to every EvidenceUploader on the page
  const uploaderRefs = useRef<any[]>([]);
  uploaderRefs.current = []; // Reset on every render
  ...
  const handleGlobalSave = async () => {
    setIsSavingAll(true);
    toast.info("Processing all updates...");
    try {
      const savePromises = uploaderRefs.current.map((ref) => {
        if (ref && ref.save) {
          return ref.save(); // Triggers the internal handleSave in EvidenceUploader
        }
      });
      await Promise.all(savePromises);
      // The individual uploaders handle their own success/fail toasts, so we just finish the loading state.
    } catch (error) {
  ...
  return (
    <main className="flex min-h-screen flex-col justify-between bg-slate-50 print:bg-white print:py-0 print:px-0">
      ...
      {/* --- PRINT ONLY FOOTER --- */}
      {/* We apply a top margin to avoid printing over the cover page header. Standard CSS print logic handles the rest. */}
                  ...
                  <div key={idx} className="bg-slate-50 p-4 rounded-xl border border-slate-200 flex flex-col justify-between space-y-4 print:mb-4 print:bg-white print:border-slate-300 print:break-inside-avoid">
                    <div>
                      <div className="flex justify-between items-start">
                        ...
                        {/* Do Not Recommend Button REMOVED here per instructions */}
                      </div>
                  ...
                  return (
                    <div key={idx} className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between space-y-4 print:mb-4 print:shadow-none print:border-slate-300 print:break-inside-avoid">
                      <div>
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <button 
                            ...
                            title="Selecting this will ensure this element will not be recommended in future plans for this student."
                          >
                      ...
                      <div className="pt-2 border-t border-slate-100">
                        {/* Amazon Links REMOVED here per instructions */}
